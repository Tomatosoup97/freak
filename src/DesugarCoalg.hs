...
coopTrans m c = case c of
    ...
    ECohandleIR (AlgTC algTName initV) c h ->
        let algTVar = "_" ++ algTName in
        let sign = hopsL h in
        ...
        let cohandle = (\c -> ECohandleIR (AlgTC algTName initV) c runner) in
...
desugarCoalg :: Comp -> Comp
desugarCoalg = coopTrans Map.empty
