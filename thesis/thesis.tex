...
means along with presentation of Freak, an experimental programming language with
(co)algebraic effects and (co)handlers. Implementation is based on Continuation
Passing Style translation.

\section{Problem Statement}\label{sec:problem-statement}

Algebraic effects, while not a new concept, have received a lot of
attention in recent years~\cite{effects-bibliography}, both from the theoretical
and practical side.
...
a lot of flexibility and have strong theoretical foundations.
...
however, they also give rise to issues that would not occur in standard
...
In the code above we open a file, and based on nondeterministic choice, we write
...
Under these circumstances one may pose a question, why we still bother with
development of the theory of programming languages, since so much has been
done already. Is there
anything that drives us towards further research? Indeed, one active branch
revolves around equational reasoning to establish equality of two programs, which we
know is undecidable in general. Proof methods may include
...
Other branch of languages theory that we shall investigate in this thesis
is about taming complexity of programs. Various methods of static analysis have
been developed, most notably, type systems. Thanks to strong
...
functional programming languages do not have them, because of their non-composable
nature.
...
How about the case of adding one more effect, $f''$ and $g''$?

It turns out that it may become complex and unpleasant to combine two monads
together to form a new monad, and for this purpose, monad
transformers~\cite{monad-transformers} arose in Haskell. Most of the usual
languages avoid this by not expressing computational effects in the type system,
and instead one may think about functions as being implicitly embedded in a Kleisli
Category of a functor $T$~\cite{kleisli}, where $T$ is a hidden signature
of all possible side effects that may occur in our program.
...
we need to develop equational theory about effects to assert correctness of
the language, as well as to have the right hammer to reason about our programs.
From practical side, we need to have \textit{the} way of taming computational effects
...
Algebraic effects can be thought of as a public interface for computational effects.
Declarative approach allows us to write programs in which the actual semantics of
source code depends on a handler that defines the meaning of a subset of effects.
...
we need to pass this interface to every function that is going to use it.
...
There are many other great sources for getting familiarized with effects from the
...
effects from the denotational point of view~\cite{bauer-what-is-algebraic}.
    ...
    For an endofunctor $F$, a $\mathVar{Free}$ $\mathVar{F-algebra}$ on an
    object $A$ (of generators) in $\mathcal{C}$ is an algebra
    ...
    \noindent
    together with a universal arrow $ \eta_{A} : A \longrightarrow A^{\#} $.
    ...
    extending $f$, i.e.\ a unique morphism in $ \mathcal{C}$ for which the diagram
    below commutes:
    ...
    % == begin Sigma-terms ==
    \begin{definition}
    A collection of $\mathVar{\Sigma-terms}$ is a free algebra with a generator $X$
    ...
    which sends variables to projections from environment and terms to
    ...
    \begin{definition} A \textit{Free model} is just a model that is a free algebra.
    \end{definition}
    ...
    \begin{proof}
        ...
        have an endofunctor $ F_{T} : \mathcal{C} \rightarrow \mathcal{C} $, which takes
        objects into free models and maps to unique homomorphisms, for which the following
        diagram commutes:
        ...
        \noindent
        which immediately gives us $\eta : 1_{C} \rightarrow F_{T}$ natural
        transformation for a monad.
        We can now define $\mu{} : {F_{T}}^{2} \rightarrow F_{T}$ for a monad
        as a collecton indexed by $X$ of unique morphisms for which
        the diagram commutes:

        \begin{center}
        \begin{tikzcd}[row sep=huge, column sep=large]
            F_{T} (X) \arrow[r, "\eta_{F_{T} (X)}"] \arrow[rd, "id"'] & F_{T} (F_{T} (X)) \arrow[d, "\mu_{F_{T}(X)}"] \\
                                                         & F_{T} (Y)
        \end{tikzcd}
        \end{center}

        \noindent
        $F_{T}$ is an endofunctor, thus $F_{T}(X)$ is an object in $\mathcal{C}$.
...
\section{Duality}\label{sec:duality}
    ...
    Due to close connections of initial algebras and free algebras, from which
    algebraic effects are arising, we spare a few words a about
    induction and coinduction, where the latter may give us more intuition
    about coalgebraic effects.

    \begin{definition}
        \textit{Initial algebra} is an initial object in \textbf{Alg}, the category of algebras
        and homomorphisms. Dually, \textit{final coalgebra} is a terminal
        object in \textbf{Alg}
    \end{definition}

    \begin{lemma}
        Free algebra over an initial object is initial algebra.
    \end{lemma}
    \begin{proof}
        Follows immediately after putting initial object as generator in the
        diagram for free algebra. Notice that from duality we have that cofree
        coalgebra over a terminal object is a final coalgebra.
    \end{proof}
    ...
    Here is a table that summarizes the differences between these two methods of reasoning:
    ...
    Recall from Chapter~\ref{chapter:background} that coalgebraic effects arises
    from cofree coalgebra. For more information about coalgebras, see~\cite{jacobs-2016}.
    ...
    \noindent
    We denote $\Lambda^{0} \subseteq \Lambda$ as a set of $\mathVar{closed~\lambda-terms}$,
...
\section{Related Work}
    ...
    effects are implemented through generators, using built-in feature of
    sending value when doing \textit{yield} operation. Implementations of effects
    based on coroutines~\cite{one-shot}, such as in Python, have one-shot and
    tail-recursive resumptions.
    ...
    the runners of comodels from T. Uustalu~\cite{runners-uustalu}.
...
state in NFA, taking the next element in an infinite stream or closing a file.
...
\end{verbatim}
    ...
    handle, with final coalgebra carrier holding \verb!<Initial state>! value.
    ...
    to the user, thus we hide it behind carrier of \textit{FileIO} coalgebraic
    theory, for which the signature is composed of \textit{Open}, \textit{Write}
    and \textit{Close} operations.
...
\end{verbatim}
    ...
    the runner of \textit{FileIO} as a first element of a pair.
...
%         let c <- do Choice () in
%         let text <- if c then return "Guy Fieri" else return "is cool" in
%         let _ <- observe Write (fh, text) in
%         observe Close fh
%     with {
%         Choose _ r ->
%             let t <- r 1 in
...
\end{verbatim}
    ...
    \begin{itemize}
        ...
        \item Validate Freak code from \verb!programs! dir: \verb!make validate_programs!
...
\section{Dynamics}
    ...
    translation. That being said, Freak introduces coalgebraic effects and a few
    basic constructs to the language, for which we shall define the semantics.
    ...
    Operationally, we embed transitions between coeffectful computations
    in a standard state monad $ S_{C}X \triangleq (C \Rightarrow X \times C) $.
    We achieve this by transforming AST of the source code, such that
    configurations are passed among cooperations. For a given coalgebraic theory
    $T$, handler of the state monad in Freak looks as follows:

\vbox{\begin{verbatim}
{
    TPut s' r -> return (\s -> let g <- r () in g s') |
    TGet _ r -> return (\s -> let g <- r s in g s) |
...
\end{verbatim}
    ...
    \noindent
    Having that defined, we expand computation
...
\end{verbatim}
    ...
    \noindent
    into the following one (modulo $\mathVar{\alpha-conversion}$):

\vbox{\begin{verbatim}
    let Tconf <- observe TGet () in
    let (TnewConf, Tres) <- observe Coeffect (Tconf, p) in
    let _ <- observe TPut TnewConf in
    return Tres
\end{verbatim}
}

    \noindent
    where before executing a coeffect, we obtain current configuration,
    pass it to coeffect, put new config and then return the actual result.
    This is done behind the scenes and the user does not know about state
    passing. Note that for readability we have done pattern matching on pairs,
    while currently in Freak it needs to be deconstructed using projections.

    \subsection{Cohandlers}

    Cohandlers needs to be aware of configuration passing, therefore,
    every cohandler accepts $ C \times P $ as a parameter where $C$ is current
    configuration and $P$ parameter passed to coeffect. In a similar way,
    value passed to resumption also needs to be a pair $ C \times X $ of new
    configuration and value returned down to computation.

    For a full currently implemented example of transformation, one may take a
    look at \verb!programs/transform/effCase! directory in source code.
...
%     TODO it's not implemented yet. Be more clear what's happening here
    ...
    The idea is that linear usages of resumptions can be easily asserted by
    analyzing parsed AST tree, where in every leaf there must be a return
    statement. It's value then can be passed into continuation of a cohandler.
    That being said, it is not implemented as of the day of submitting the
    thesis.
