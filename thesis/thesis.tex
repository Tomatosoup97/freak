...
\section{Dynamics}
    ...
    \subsection{Cohandlers}\label{subsec:calc-cohandlers}

    Semantics for cohandlers is brought from denotational one that we have
    introduced in Section~\ref{sec:duality}, meaning, we denote cooperations
    ...
    Operationally, we embed transitions between coeffectull computations
...
\vbox{\begin{verbatim}
    ...
    let Tcoresult <- observe Coeffect (Tconf, p) in
    let TnewConf <- return fst Tcoresult in
    let Tres <- return snd Tcoresult in
