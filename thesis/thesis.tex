...
For the purpose of describing the duality between algebra and coalgebra,
we allow ourselves to wander a bit deeper into category theory.

    \subsection{Algebraic Theories}

    \begin{definition}{\mathVar{Signature}}
    ...
    \begin{definition}{$\mathVar{\Sigma-terms}$}
    ...
    \begin{definition}{$ \mathVar{\Sigma-Equation} $}
    ...
    that a given $\mathVar{\Sigma-term}$ $X$ $|$ $t$ is interpreted by a map

    $ {\llbracket X | t \rrbracket}_I : {|I|}^{k} \rightarrow |I| $

    TODO actually define the map as a projection on variables and
    map composition on compound terms.

    where ${|I|}^{k}$ is an k-ary product in our category, assuming that all of the
    necessary products exist.

    \end{definition}

    TODO Models, Free models

    \subsection{Free models form monads}

    Maybe not a subsection but just a lemma

    \subsection{Models homomorphisms}

    T-homomorphisms with respect to theory T

    \subsection{Parametrized operations}
